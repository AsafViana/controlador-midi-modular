...
void UnifiedControlList::rebuild() {
  ...
  for (uint8_t i = 0; i < HardwareMap::NUM_CONTROLES; i++) {
    if (_numControles >= MAX_TOTAL_CONTROLS)
      break;
    ...
    ctrl.ccPadrao = HardwareMap::getCCPadrao(i);
    ctrl.isRemoto = false;
...
uint8_t UnifiedControlList::getNumControles() const { return _numControles; }

// ── getNumLocais() ───────────────────────────────────────

uint8_t UnifiedControlList::getNumLocais() const { return _numLocais; }

// ── getControlInfo() ─────────────────────────────────────

bool UnifiedControlList::getControlInfo(uint8_t index, ControlInfo &out) const {
  if (index >= _numControles)
    return false;
  out = _controls[index];
  return true;
}
...
bool UnifiedControlList::getRemoteInfo(uint8_t index, uint8_t &address,
                                       uint8_t &ctrlIdx) const {
  if (index >= _numControles)
    return false;
  if (!_controls[index].isRemoto)
    return false;
