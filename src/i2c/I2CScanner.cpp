...
uint8_t I2CScanner::scan() {
    ...
    for (uint8_t addr = I2CProtocol::ADDR_MIN; addr <= I2CProtocol::ADDR_MAX; addr++) {
        // Pular endereço do OLED
        if (addr == I2CProtocol::ADDR_OLED) continue;

        // Limite de módulos atingido
        if (_moduleCount >= MAX_MODULES) break;
...
void I2CScanner::periodicScan() {
    ...
    if (now - _lastScanTime < RESCAN_INTERVAL_MS) return;

    _lastScanTime = now;

    // 1. Verificar módulos já conhecidos — detectar desconexões
    for (uint8_t i = 0; i < _moduleCount; i++) {
        if (!_modules[i].connected) continue;
...
bool I2CScanner::readValues(uint8_t moduleIndex, uint8_t* values, uint8_t maxLen) {
    if (moduleIndex >= _moduleCount) return false;

    ModuleInfo& info = _modules[moduleIndex];
    if (!info.connected) return false;

    uint8_t numControles = info.descriptor.numControles;
    if (numControles > maxLen) numControles = maxLen;
...
uint8_t I2CScanner::getModuleCount() const {
    return _moduleCount;
}
...
const ModuleInfo* I2CScanner::getModule(uint8_t index) const {
    if (index >= _moduleCount) return nullptr;
...
bool I2CScanner::probeAndRead(uint8_t address, ModuleDescriptor& desc) {
    ...
    if (!_bus->probe(address)) return false;

    // 2. Enviar comando CMD_DESCRIPTOR
    uint8_t cmd = I2CProtocol::CMD_DESCRIPTOR;
    if (!_bus->write(address, &cmd, 1)) return false;

    // 3. Ler resposta (tamanho máximo: 1 + 16*14 = 225 bytes)
    uint8_t buffer[225];
    uint8_t bytesRead = _bus->requestFrom(address, buffer,
                                          static_cast<uint8_t>(sizeof(buffer)));
    if (bytesRead == 0) return false;

    // 4. Desserializar
    if (!I2CProtocol::deserialize(buffer, bytesRead, desc)) return false;
...
void I2CScanner::registerFailure(uint8_t moduleIndex) {
    if (moduleIndex >= _moduleCount) return;
