...
bool OledApp::begin(uint8_t i2cAddress) {
    // Wire.begin() já foi chamado por WireI2CBus::begin() no setup().
    // Aqui apenas instanciamos o display usando o barramento existente.
...
void OledApp::showSplash(const char* nome, const char* versao, uint16_t duracaoMs) {
...
void OledApp::update() {
    ...
    if (_btnUp) {
        ...
        else if (ev == ButtonEvent::LONG_PRESS) _router.handleInput(NavInput::LONG_UP);
    }
    if (_btnDown) {
        ButtonEvent ev = _btnDown->update();
        if (ev == ButtonEvent::PRESSED)     _router.handleInput(NavInput::DOWN);
        else if (ev == ButtonEvent::LONG_PRESS) _router.handleInput(NavInput::LONG_DOWN);
    }
    if (_btnSelect) {
        ButtonEvent ev = _btnSelect->update();
        if (ev == ButtonEvent::PRESSED) _router.handleInput(NavInput::SELECT);
    }
