...
void TextComponent::render(Adafruit_SSD1306 &display) {
  ...
  if (static_cast<int16_t>(_x + textW) <= MAX_DISPLAY_WIDTH) {
    ...
    // Find the longest substring that fits within MAX_DISPLAY_WIDTH
    while (len > 0) {
      len--;
      if (len == 0) {
      ...
      size_t copyLen =
          (len < sizeof(truncated) - 1) ? len : sizeof(truncated) - 1;
...
void TextComponent::setText(const char *text) { _text = text; }
