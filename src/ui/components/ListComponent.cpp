...
void ListComponent::render(Adafruit_SSD1306 &display) {
  ...
  int16_t arrowX = _x + _w - 6; // 6px da margem direita
  display.setTextColor(SSD1306_WHITE);
  display.setTextSize(1);

  if (_scrollOffset > 0) {
    // Há itens acima — desenha ▲
    display.setCursor(arrowX, _y);
    display.print("^");
  }
  if (_scrollOffset + _visibleCount < _itemCount) {
    // Há itens abaixo — desenha ▼
    display.setCursor(arrowX, _y + _h - 8);
    display.print("v");
  }
}
...
uint8_t ListComponent::getSelectedIndex() const { return _selectedIndex; }

void ListComponent::setUpButton(ButtonEvent upEvent) { _upEvent = upEvent; }
