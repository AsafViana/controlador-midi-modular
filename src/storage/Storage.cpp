...
void Storage::begin() {
  // Inicializa defaults a partir do HardwareMap
  for (uint8_t i = 0; i < MAX_CONTROLES; i++) {
    _ccMap[i] = HardwareMap::getCCPadrao(i);
    _ccHabilitado[i] = true; // Todos habilitados por padrão
  }
  _tecladoHabilitado = true;

  // Inicializa cache de configurações remotas (tudo vazio)
...
uint8_t Storage::getRemoteCC(uint8_t i2cAddr, uint8_t ctrlIdx) const {
  ...
  return 0; // Default CC = 0 (no descriptor available here)
}
...
#ifdef ARDUINO
  // Regravar defaults no NVS para consistência
