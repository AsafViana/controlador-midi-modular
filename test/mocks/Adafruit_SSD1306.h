...
public:
     ...
     * fontSize 1 = 6px per char, fontSize 2 = 12px per char, fontSize 3 = 18px per char.
     * Height: fontSize 1 = 8px, fontSize 2 = 16px, fontSize 3 = 24px.
     */
    void getTextBounds(const char* str, int16_t x, int16_t y,
        ...
        if (x1) *x1 = x;
        if (y1) *y1 = y;
        if (w) *w = len * charWidth;
        if (h) *h = (len > 0) ? charHeight : 0;
    }
