...
public:
  void sendNoteOn(MIDIAddress addr, uint8_t velocity) {
    mock_midi::lastMessage = {addr.note, addr.channel, velocity, true, 0,
                              0,         false};
    mock_midi::messageCount++;
  }

  void sendNoteOff(MIDIAddress addr, uint8_t velocity) {
    mock_midi::lastMessage = {addr.note, addr.channel, velocity, false, 0,
                              0,         false};
    mock_midi::messageCount++;
  }

  void sendControlChange(MIDIAddress addr, uint8_t value) {
    mock_midi::lastMessage = {0,         addr.channel, 0,   false,
                              addr.note, value,        true};
    mock_midi::messageCount++;
  }
