...
inline uint32_t micros() { return mock::currentMillis * 1000; }

inline void pinMode(uint8_t pin, uint8_t mode) { mock::pinModes[pin] = mode; }

inline int digitalRead(uint8_t pin) { return mock::pinValues[pin]; }
...
inline int analogRead(uint8_t pin) { return mock::pinValues[pin]; }
