 ...
 *
 * Feature: i2c-modular-expansion, Property 1: Modo standalone preserva
 * comportamento local Feature: i2c-modular-expansion, Property 6: Ordenação da
 * lista unificada — locais primeiro, remotos depois
 *
 ...
 * valid local index:
 *   - getLabel() must return the same pointer as
 * HardwareMap::CONTROLES[idx].label
 *   - getTipo() must return the same value as HardwareMap::CONTROLES[idx].tipo
 *   - getCCPadrao() must return the same value as
 * HardwareMap::CONTROLES[idx].ccPadrao
...
void test_property1_standalone_preserves_local() {
  ...
  for (int i = 0; i < PBT_ITERATIONS; i++) {
    ...
    uint8_t hwCC = HardwareMap::getCCPadrao(idx);
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(
        hwCC, uclCC, "Standalone: ccPadrao must match HardwareMap");
...
/**
 * Feature: i2c-modular-expansion, Property 6: Ordenação da lista unificada —
 * locais primeiro, remotos depois
 *
...
void test_property6_ordering_locals_first() {
  ...
  const uint8_t validAddresses[] = {0x20, 0x21, 0x22, 0x23,
                                    0x24, 0x25, 0x26, 0x27};
  ...
  for (int iter = 0; iter < PBT_ITERATIONS; iter++) {
    ...
    for (uint8_t m = 0; m < numModules; m++) {
      ...
      if (totalRemoteControls + numCtrl + HardwareMap::NUM_CONTROLES >
          UnifiedControlList::MAX_TOTAL_CONTROLS) {
        numCtrl = UnifiedControlList::MAX_TOTAL_CONTROLS -
                  HardwareMap::NUM_CONTROLES - totalRemoteControls;
        if (numCtrl == 0)
          break;
      }
    ...
    for (uint8_t i = numLocais; i < numTotal; i++) {
      TEST_ASSERT_TRUE_MESSAGE(ucl.isRemoto(i),
                               "Property 6: indices numLocais..numTotal-1 must "
                               "be remote (isRemoto==true)");
    }

    // Verify total count is consistent
    TEST_ASSERT_TRUE_MESSAGE(numTotal >= numLocais,
...
/**
 * Feature: i2c-modular-expansion, Property 7: Dados de controles remotos
 * correspondem ao descritor
 *
  ...
  for (int iter = 0; iter < PBT_ITERATIONS; iter++) {
    ...
    for (uint8_t i = numLocais; i < numTotal; i++) {
      ...
      TEST_ASSERT_TRUE_MESSAGE(
          ok, "Property 7: getControlInfo must succeed for valid remote index");
      ...
      for (uint8_t e = 0; e < expectedCount; e++) {
        if (expected[e].moduleAddress == addr && expected[e].ctrlIdx == cIdx) {
          ...
          // Verify tipo
          TEST_ASSERT_EQUAL_UINT8_MESSAGE(
              expected[e].tipo, static_cast<uint8_t>(ucl.getTipo(i)),
              "Property 7: tipo must match descriptor");

          // Verify ccPadrao (= valor from descriptor)
          TEST_ASSERT_EQUAL_UINT8_MESSAGE(
              expected[e].valor, ucl.getCCPadrao(i),
