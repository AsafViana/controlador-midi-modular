...
public:
  int mountCount = 0;
  int unmountCount = 0;
  NavInput lastInput = NavInput::NONE;
  bool backHandled = false;

  void onMount() override { mountCount++; }
  void onUnmount() override { unmountCount++; }
  void render(Adafruit_SSD1306 &display) override {}
  void handleInput(NavInput input) override { lastInput = input; }
  bool handleBack() override { return backHandled; }
};
...
void test_handle_input_forwards_to_current_screen() {
  ...
  TestScreen screen;
  router.push(&screen);
  router.handleInput(NavInput::SELECT);
  TEST_ASSERT_EQUAL((int)NavInput::SELECT, (int)screen.lastInput);
}

void test_handle_input_with_empty_stack_does_not_crash() {
  Router router;
  router.handleInput(NavInput::UP); // Should not crash
}
...
void test_handle_input_forwards_various_events() {
  ...
  router.handleInput(NavInput::LONG_UP);
  TEST_ASSERT_EQUAL((int)NavInput::LONG_UP, (int)screen.lastInput);

  router.handleInput(NavInput::DOWN);
  TEST_ASSERT_EQUAL((int)NavInput::DOWN, (int)screen.lastInput);
}

// --- BACK button tests ---

void test_back_pops_when_screen_does_not_handle() {
  Router router;
  TestScreen screenA;
  TestScreen screenB;
  screenB.backHandled = false; // Screen does NOT handle BACK
  router.push(&screenA);
  router.push(&screenB);
  router.handleInput(NavInput::BACK);
  TEST_ASSERT_EQUAL_PTR(&screenA, router.currentScreen());
}

void test_back_does_not_pop_when_screen_handles() {
  Router router;
  TestScreen screenA;
  TestScreen screenB;
  screenB.backHandled = true; // Screen DOES handle BACK
  router.push(&screenA);
  router.push(&screenB);
  router.handleInput(NavInput::BACK);
  TEST_ASSERT_EQUAL_PTR(&screenB, router.currentScreen());
}

void test_back_on_root_screen_does_nothing() {
  Router router;
  TestScreen screen;
  screen.backHandled = false;
  router.push(&screen);
  router.handleInput(NavInput::BACK);
...
int main() {
  ...
  // BACK button
  RUN_TEST(test_back_pops_when_screen_does_not_handle);
  RUN_TEST(test_back_does_not_pop_when_screen_handles);
  RUN_TEST(test_back_on_root_screen_does_nothing);
