...
void test_rebuild_no_modules() {
    ...
    for (uint8_t i = 0; i < HardwareMap::NUM_CONTROLES; i++) {
        ...
        TEST_ASSERT_EQUAL_UINT8_MESSAGE(
            HardwareMap::CONTROLES[i].ccPadrao, ucl.getCCPadrao(i),
...
void test_rebuild_max_controls() {
    ...
    uint8_t expectedRemotes = UnifiedControlList::MAX_TOTAL_CONTROLS - HardwareMap::NUM_CONTROLES;
    ...
    for (uint8_t i = HardwareMap::NUM_CONTROLES; i < ucl.getNumControles(); i++) {
        TEST_ASSERT_TRUE_MESSAGE(
            ucl.isRemoto(i),
            "Max controls: indices after locals must be remote");
    }
    ...
    TEST_ASSERT_FALSE_MESSAGE(
        ...
        "Max controls: getControlInfo must return false for index >= MAX_TOTAL_CONTROLS");
}
...
void test_rebuild_after_disconnect() {
    ...
    TEST_ASSERT_FALSE_MESSAGE(
        mod0x20->connected,
        "After 3 failures: module 0x20 must be disconnected");
